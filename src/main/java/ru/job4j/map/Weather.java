...
public class Weather {
    public static List<Info> editData(List<Info> list) {
        ...
        for (Info myCity: list) {
            cities.computeIfPresent(myCity.getCity(), (city, rainfall) -> rainfall + myCity.getRainfall());
            cities.putIfAbsent(myCity.getCity(), myCity.getRainfall());
        }
        for (String city: cities.keySet()) {
            rsl.add(new Info(city, cities.get(city)));
        }
        return rsl;
    }
